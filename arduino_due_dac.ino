  ...
  if(Serial.available()){
    switch(Serial.read()){
      ...
      case'f':{
        ...
        double period = 1.0L / freq;

        double microsec = period * 100000.0L / 12.0L;

        //String send = millis() + "frequenza : " + freq + "Hz, periodo : " + period + "ms";
/*
        ...
        Serial.print(microsec - MSECBASE, 9); Serial.println("us");
*/
        delayusec = microsec - MSECBASE;
  ...
  //analogWrite(DAC0, buf[i]);  // write the selected waveform on DAC0
  //analogWrite(DAC1, waveformsTable[0][i]);  // write the selected waveform on DAC1

  //time1 = micros();
  
  analogWrite(DAC1, buf[i]);  // write the selected waveform on DAC0

  i++;
  if(i == sample)  // Reset the counter to repeat the wave
    i = 0;

  //time2 = micros();

  //Serial.println(time2-time1);
  delayMicroseconds(delayusec);  // Hold the sample value for the sample time
}
